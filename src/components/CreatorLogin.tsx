...
const CreatorLogin: React.FC<CreatorLoginProps> = ({ setView, goBack }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      localStorage.setItem('currentCreator', username);
      // Generate or reuse a unique creatorId for this username
      let creatorId = localStorage.getItem(`creatorId:${username}`);
      if (!creatorId) {
        // Simple random string (could use uuid in a real app)
        creatorId = Math.random().toString(36).substring(2, 12) + Date.now().toString(36);
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 relative z-10">
        <div className="relative rounded-3xl shadow-xl p-10 max-w-md w-full bg-white/80 flex flex-col items-center justify-center border-4 border-[#CFFFE5]" style={{ boxShadow: '0 8px 32px #a2d2ff33' }}>
          ...
          <p className="font-quicksand text-base text-[#232946] mb-6 animate-fade-in relative" style={{ maxWidth: 320 }}>
            Access your account to manage quizzes and view analytics.
          ...
          <div className="w-full flex flex-col gap-4 mt-4">
            ...
            >
              {loading ? 'Signing In...' : <><MdAccountCircle className="inline text-2xl mr-2 align-middle" />Sign In</>}
            </button>
            <button
              onClick={goBack}
          ...
          <div className="mt-6 text-center text-[#232946] text-xs font-quicksand opacity-70">
            Don&apos;t have an account? Just enter a username and password to create one instantly.
          </div>
        </div>
      </main>
      {/* Footer */}
