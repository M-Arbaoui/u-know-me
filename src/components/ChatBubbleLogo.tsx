...
const ChatBubbleLogo: React.FC<ChatBubbleLogoProps> = ({ 
  size = 'md', 
  variant = 'full', 
  animated = true,
  className = '' 
  ...
  const ChatBubbleIcon = () => (
    <div className={`${iconSizes[size]} relative group`}>
      ...
      <div className="w-full h-full relative">
        {/* Bubble body */}
        <div className="w-full h-4/5 bg-gradient-to-br from-purple-600 via-pink-500 to-blue-500 rounded-2xl flex items-center justify-center shadow-lg">
          <div className="w-3/4 h-3/4 bg-white/20 rounded-xl flex items-center justify-center">
            <span className="text-white text-xs font-black">💬</span>
          </div>
        </div>
        
        {/* Bubble tail */}
        <div className="absolute bottom-0 left-1/4 w-1/2 h-1/5 bg-gradient-to-br from-purple-600 to-pink-500 rounded-b-full"></div>
        
        {/* XP Bar */}
        <div className="absolute -bottom-1 left-0 right-0 h-1 bg-white/20 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-yellow-400 via-orange-500 to-red-500 rounded-full animate-pulse-gentle" style={{ width: '75%' }}></div>
        </div>
